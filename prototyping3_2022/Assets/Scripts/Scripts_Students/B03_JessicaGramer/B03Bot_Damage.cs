...
[RequireComponent(typeof(B03Bot_Weapon))]
public class B03Bot_Damage : MonoBehaviour
{
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		if ((other.gameObject.tag == "Hazard") && (notMyWeapon == true))
		{
			...
			if (Vector3.Dot(transform.forward, directionFore) < (-sidelimit)) // back sheilds
			{
				...
				else
				{
					...
					else DamageEvent.Invoke();
				}
			}

			else if (Vector3.Dot(transform.forward, directionFore) > sidelimit) // front sheilds
			{
				...
				if(GetComponent<B03Bot_Weapon>().currentWeapon == B03Bot_Weapon.Weapon.SHEILD)
                {
					GetComponent<B03Bot_Weapon>().ActivateWeapon.Invoke(B03Bot_Weapon.Weapon.NONE);
                }
				else if (shieldPowerFront <= 0)
				{
