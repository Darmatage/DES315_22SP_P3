...
public class TrapBot_Damage : MonoBehaviour
{
	...
	private bool already_damaged = false;
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		if ((other.gameObject.tag == "Hazard") && (notMyWeapon == true))
		{
			...
			if (Vector3.Dot(transform.forward, directionFore) < (-sidelimit) && already_damaged == false)
			{
			...
			else if (Vector3.Dot(transform.forward, directionFore) > sidelimit && already_damaged == false)
			{
			...
			else if (Vector3.Dot(compassSides.transform.forward, directionSides) > sidelimit && already_damaged == false)
			{
			...
			else if (Vector3.Dot(compassVertical.transform.forward, directionVert) > sidelimit && already_damaged == false)
			{
